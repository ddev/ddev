...
# Ignore notices about missing Signed-By
if grep -iE "warning.*signature|audit.*signature|error.*signature|SHA1.*not.*secure" /tmp/apt-audit.log | grep -v "Missing Signed-By"; then
