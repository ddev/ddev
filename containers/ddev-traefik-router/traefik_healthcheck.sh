...
if [ $exit_code -eq 0 ]; then
    ...
    if [ "$expected_router_count" -eq 0 ]; then
        check="No router config files found"
        exit_code=1
    elif [ "$file_router_count" -ne "$expected_router_count" ]; then
        check="Router count mismatch: ${file_router_count} loaded, ${expected_router_count} expected"
        exit_code=2
    elif [ "$error_count" -gt 0 ]; then
        check="Detected ${error_count} configuration error(s) in project"
        exit_code=3
    elif [ "$services_not_up" -gt 0 ]; then
        check="Detected ${services_not_up} service(s) not yet UP"
        exit_code=4
    else
