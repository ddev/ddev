...
timeout "${START_SCRIPT_TIMEOUT:-30}" n install "${N_INSTALL_VERSION}" \
  || log-stderr.sh --timeout "${START_SCRIPT_TIMEOUT:-30}" n install "${N_INSTALL_VERSION}" \
  || n_install_result=false
