...
if [ -d ${ENTRYPOINT} ] && ([ "${DDEV_PROJECT_TYPE}" = "django4" ] || [ "${DDEV_PROJECT_TYPE}" = "python" ]) || ([[ -n $(find ${ENTRYPOINT} -type f -regex ".*\.\(sh\)") ]] && [[ ! -f "${ENTRYPOINT}/.user_scripts_initialized" ]]) ; then
...
# Run any python/django4 activities; Under mutagen these may have required the
# quick setup of /var/www/html shown above. This has to be done before gunicorn
# starts up though.
ddev_python_setup
