...
@test "verify a directly-requested nonexistent .php file for ${WEBSERVER_TYPE} php${PHP_VERSION}" {
  # A URL for a .php file that doesn't exist on disk must be rejected before
  # it ever reaches php-fpm, so ddev-webserver's explanatory 404 is shown
  # instead of php-fpm's own "No input file specified" response:
  #   - nginx: `try_files $uri =404;` in the `\.php$` location block.
  #   - apache: the `-f %{REQUEST_FILENAME}` guard on the FilesMatch handler
  #     for *.php/.phar/.phtml.
