...
func TestDebugGobDecodeWithRealCacheFiles(t *testing.T) {
	...
	t.Run("RemoteConfig", func(t *testing.T) {
		...
		if err != nil {
			t.Skipf("Real remote config file could not be decoded, likely written by an older ddev version: %v", err)
		}
	...
	t.Run("SponsorshipData", func(t *testing.T) {
		...
		out, err := exec.RunHostCommandSeparateStreams(DdevBin, "utility", "gob-decode", filePath)
		if err != nil {
			t.Skipf("Real sponsorship data file could not be decoded, likely written by an older ddev version: %v", err)
		}
