...
func TestCmdGlobalConfig(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		args := []string{"config", "global", "--omit-containers", "", "--performance-mode-reset", "--simple-formatting=false", "--table-style=default", `--required-docker-buildx-version=""`, `--xdebug-ide-location`, "", `--traefik-monitor-port=10999`}
