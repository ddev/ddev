...
func TestCmdGlobalConfig(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		args := []string{"config", "global", "--omit-containers", "", "--disable-http2=false", "--performance=", "--simple-formatting=false", "--table-style=default", `--required-docker-compose-version=""`, `--use-docker-compose-from-path=false`, `--xdebug-ide-location`, "", `--router=traefik`}
	...
	assert.Contains(out, fmt.Sprintf("performance=%s", configTypes.GetPerformanceDefault()))
	...
	assert.Contains(out, fmt.Sprintf("performance=%s", configTypes.PerformanceMutagen))
	...
	args = []string{"config", "global", "--performance=nfs"}
	out, err = exec.RunCommand(DdevBin, args)
	assert.NoError(err)

	assert.Contains(string(out), "performance=nfs")
