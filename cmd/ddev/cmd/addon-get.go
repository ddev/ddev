...
ddev add-on get ddev/ddev-redis --latest
...
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		case len(parts) == 2: // github.com/owner/repo
			...
			tarballURL, downloadedRelease, err = ddevapp.GetAddonTarballURL(sourceRepoArg, requestedVersion, useLatest, prNumber)
			if err != nil {
				util.Failed("%v", err)
			}
...
func init() {
	...
	AddonGetCmd.Flags().Bool("latest", false, "Install from the default branch instead of the latest release")
	_ = AddonGetCmd.RegisterFlagCompletionFunc("latest", configCompletionFunc([]string{"true", "false"}))
	...
	AddonGetCmd.MarkFlagsMutuallyExclusive("version", "latest", "pr")
