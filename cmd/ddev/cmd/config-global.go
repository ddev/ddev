...
func init() {
	...
	configGlobalCommand.Flags().BoolVarP(&instrumentationOptIn, "instrumentation-opt-in", "", true, "instrumentation-opt-in=true")
	...
	configGlobalCommand.Flags().String("table-style", "default", fmt.Sprintf(`Table style for list and describe, possible values are "%s"`, strings.Join(globalconfig.ValidTableStyleList(), `", "`)))
	_ = configGlobalCommand.RegisterFlagCompletionFunc("table-style", configCompletionFunc(globalconfig.ValidTableStyleList()))
	configGlobalCommand.Flags().String("required-docker-compose-version", versionconstants.RequiredDockerComposeVersionDefault, "Override default docker-compose version (used only in development testing)")
	_ = configGlobalCommand.Flags().MarkHidden("required-docker-compose-version")
	configGlobalCommand.Flags().String("project-tld", nodeps.DdevDefaultTLD, "Override default project tld")
	...
	configGlobalCommand.Flags().Bool("wsl2-no-windows-hosts-mgt", false, "WSL2 only; make DDEV ignore Windows-side hosts file")
	configGlobalCommand.Flags().String("router-http-port", nodeps.DdevDefaultRouterHTTPPort, "The default router HTTP port for all projects")
	...
	configGlobalCommand.Flags().String("router-https-port", nodeps.DdevDefaultRouterHTTPSPort, "The default router HTTPS port for all projects")
	...
	configGlobalCommand.Flags().String("mailpit-http-port", nodeps.DdevDefaultMailpitHTTPPort, "The default Mailpit HTTP port for all projects")
	...
	configGlobalCommand.Flags().String("mailpit-https-port", nodeps.DdevDefaultMailpitHTTPSPort, "The default Mailpit HTTPS port for all projects")
	...
	configGlobalCommand.Flags().String("traefik-monitor-port", nodeps.TraefikMonitorPortDefault, "The Traefik monitor port; can be changed in case of port conflicts")
