...
func handleGlobalConfig(cmd *cobra.Command, args []string) {
	...
	if cmd.Flag("disable-router").Changed {
		val, _ := cmd.Flags().GetBool("disable-router")
		globalconfig.DdevGlobalConfig.DisableRouter = val
		dirty = true
	}
	...
	output.UserOut.Printf("disable-router=%v", globalconfig.DdevGlobalConfig.DisableRouter)
...
func init() {
	...
	configGlobalCommand.Flags().Bool("disable-router", false, "Disable ddev-router so another ")
