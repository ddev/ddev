...
func cleanupTestDistro(t *testing.T) {
	...
	out, err := exec.RunHostCommand("bash", `-c`, `wsl.exe -l -q`)
	if err != nil {
...
func createTestWSL2Distro(t *testing.T) {
	...
	_, err = exec.RunHostCommand("wsl.exe", "-d", testDistroName, "-u", "root", "bash", "-c", "useradd -m -s /bin/bash testuser && echo 'testuser:testpass' | chpasswd && usermod -aG sudo testuser")
	require.NoError(err, "Failed to create test user: %v", err)
...
func testBasicDdevFunctionality(t *testing.T) {
	...
	out, err = exec.RunHostCommand("wsl.exe", "-d", testDistroName, "bash", "-c", fmt.Sprintf("cd %s && ddev config --project-type=html --project-name=test-project --auto", projectDir))
	require.NoError(err, "ddev config failed: %v, output: %s", err, out)
	t.Logf("ddev config output: %s", out)

	// Start the project
	out, err = exec.RunHostCommand("wsl.exe", "-d", testDistroName, "bash", "-c", fmt.Sprintf("cd %s && ddev start", projectDir))
	...
	_, _ = exec.RunHostCommand("wsl.exe", "-d", testDistroName, "bash", "-c", fmt.Sprintf("cd %s && ddev stop", projectDir))
