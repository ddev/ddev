...
!define PRODUCT_AWESOME_DDEV_URL "https://github.com/drud/awesome-ddev#readme"

!define PRODUCT_DDEV_CONTRIB "${PRODUCT_NAME} Services & Snippets"
!define PRODUCT_DDEV_CONTRIB_URL "https://github.com/drud/ddev-contrib#readme"
...
Caption "${PRODUCT_NAME_FULL} ${PRODUCT_VERSION} Setup"
...
SectionGroup /e "${PRODUCT_NAME_FULL}"
  ...
  Section "${PRODUCT_NAME_FULL}" SecDDEV
    ...
    ; Awesome DDEV
    WriteIniStr "$INSTDIR\Links\${PRODUCT_AWESOME_DDEV}.url" "InternetShortcut" "URL" "${PRODUCT_AWESOME_DDEV_URL}"
    CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\${PRODUCT_AWESOME_DDEV}.lnk" "$INSTDIR\Links\${PRODUCT_AWESOME_DDEV}.url" "" "$INSTDIR\Icons\ddev.ico"

    ; DDEV Contrib
    WriteIniStr "$INSTDIR\Links\${PRODUCT_DDEV_CONTRIB}.url" "InternetShortcut" "URL" "${PRODUCT_DDEV_CONTRIB_URL}"
    CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\${PRODUCT_DDEV_CONTRIB}.lnk" "$INSTDIR\Links\${PRODUCT_DDEV_CONTRIB}.url" "" "$INSTDIR\Icons\ddev.ico"
...
SectionGroup /e "mkcert"
  ...
  Section "mkcert" SecMkcert
    ...
    CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\mkcert\Install local CA.lnk" "$INSTDIR\mkcert install.lnk"
    CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\mkcert\Uninstall local CA.lnk" "$INSTDIR\mkcert uninstall.lnk"
    !insertmacro MUI_STARTMENU_WRITE_END
  SectionEnd
...
/**
 * Disable sudo license pages if component is not selected
 */
Function sudoLicPre
  ${IfNot} ${SectionIsSelected} ${SecSudo}
    Abort
  ${EndIf}
FunctionEnd

/**
 * Disable mkcert license pages if component is not selected
 */
Function mkcertLicPre
  ${Unless} ${SectionIsSelected} ${SecMkcert}
    Abort
  ${EndIf}
FunctionEnd

/**
 * Disable WinNFSd license pages if component is not selected
 */
Function winNFSdLicPre
  ${IfNot} ${SectionIsSelected} ${SecWinNFSd}
    Abort
  ${EndIf}
FunctionEnd
