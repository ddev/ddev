...
# emit_metric <name> <value_ms> prints one JSON result line to stdout.
# value_ms may be the literal "null" when a metric doesn't apply (e.g. Mutagen disabled).
emit_metric() {
  local name="$1" value="$2"
  jq -n --arg metric "$name" --argjson value_ms "$value" '{metric: $metric, value_ms: $value_ms}'
}
