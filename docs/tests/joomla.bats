...
@test "Joomla quickstart with $(ddev --version)" {
  ...
  tag=$(curl -L "https://api.github.com/repos/joomla/joomla-cms/releases/latest"

  run docker run -i --rm ddev/ddev-utilities jq -r .tag_name) && curl -L "https://github.com/joomla/joomla-cms/releases/download/$tag/Joomla_$tag-Stable-Full_Package.zip" -o joomla.zip
  assert_success
