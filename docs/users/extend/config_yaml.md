...
| composer_version | version of composer to use in web container and `ddev composer` | It can be left blank to get the default for the current ddev version, or set to "1", for latest composer v1, "2" for latest composer v2, or to an explicit composer version. |
