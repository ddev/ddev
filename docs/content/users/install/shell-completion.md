...
=== "macOS Bash + Homebrew"
    ...
    !!!note "Bash profile"
        You must add the include to your `.bash_profile` or `.profile` or nothing will work. Use `source ~/.bash_profile` or `source ~/.profile` to make it take effect immediately. You will also need to reload your shell either by reopening your terminal window or executing `exec zsh` or `exec bash`
...
=== "Bash/Zsh/Fish on Linux"
    ...
    3. Remember to reload your shell with `exec zsh` or `exec bash` or by closing and reopening your terminal.
