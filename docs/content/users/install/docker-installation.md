...
* If you see the error message "your Docker API version (A.B) doesn't meet the required minimum (X.Y)," you need to upgrade your Docker engine. The best way to do this is by updating Docker from the official Docker repository. If you're using a version from your distribution's default package repository, it's likely outdated. In that case, uninstall Docker Engine and follow the installation instructions above to install a supported version from the official Docker repository.
