...
2. Install the certificate and key in your project’s `.ddev/custom_certs` directory.
   ...
   * If you’re [using Traefik](../configuration/traefik-router.md#traefik-router), the files should be named `<projectname>.crt` and `<projectname>.key`, for example `exampleproj.crt` and `exampleproj.key`.
