...
Team members may choose to use `config.local.yaml` for local non-committed config changes, for example. `config.*.local.yaml` files can also be used to modify the order in which the local overrides are loaded. `config.local.yaml` and `config.*.local.yaml` are gitignored by default.
