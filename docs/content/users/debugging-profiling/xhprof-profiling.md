...
* Enable xhprof with [`ddev xhprof on`](../basics/commands.md#xhprof) (or `ddev xhprof` or `ddev xhprof enable`) and check its status with `ddev xhprof status`.
...
* The runs are erased on [`ddev restart`](../basics/commands.md#restart).
