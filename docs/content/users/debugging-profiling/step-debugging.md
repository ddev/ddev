...
* **WSL2 VirtioProxy Networking Mode (experimental):** If you're using `networkingMode=VirtioProxy` in your Windows-side `~/.wslconfig`, DDEV supports Xdebug experimentally. On systems where Windows creates a `vEthernet (WSL)` Hyper-V adapter, DDEV automatically discovers the Windows host IP and a Windows-side IDE works. On systems without that adapter (notably ARM64), Docker containers have no network path to the Windows host; in that case run your IDE inside WSL2: `ddev config global --xdebug-ide-location=wsl2`. Run `ddev utility xdebug-diagnose` to check your configuration.
