...
* If you want to use xdebug with `ddev drush` you need to add the `DRUSH_ALLOW_XDEBUG=1` [environment variable](../extend/customization-extendibility/#environment-variables-for-containers-and-services), thus making all drush commands slower as they are run with xdebug enabled. You can also enable it for a single drush command by running: `ddev exec 'DRUSH_ALLOW_XDEBUG=1 drush st'`. For `drush` 13+, you can also use the [global `--xdebug` option for drush](https://www.drush.org/13.x/commands/#xdebug): `ddev drush --xdebug st`. 
