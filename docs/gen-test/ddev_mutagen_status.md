...
{% raw %}## ddev mutagen status

Shows mutagen sync status

```
ddev mutagen status [flags]
```
...
<dl class="flags">
	<dt><code>-l</code>, <code>--verbose</code></dt>
	<dd>Extended/verbose output for mutagen status</dd>
</dl>
...
{% highlight bash %}{% raw %}
"ddev mutagen status", "ddev mutagen status <projectname>"{% endraw %}{% endhighlight %}
