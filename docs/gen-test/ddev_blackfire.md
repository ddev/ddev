...
Enable or disable blackfire.io profiling (global shell web container command)

```
ddev blackfire start|stop|on|off|enable|disable|true|false|status
```
...
{% highlight bash %}{% raw %}
  "ddev blackfire" (default is "on"), "ddev blackfire off", "ddev blackfire on", "ddev blackfire status"{% endraw %}{% endhighlight %}
