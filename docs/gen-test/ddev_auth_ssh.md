...
Use this command to provide the password to your ssh key to the ddev-ssh-agent container, where it can be used by other containers. Normal usage is just "ddev auth ssh", or if your key is not in ~/.ssh, ddev auth ssh --ssh-key-path=/some/path/.ssh"
...
<dl class="flags">
	<dt><code>-d</code>, <code>--ssh-key-path &lt;string&gt;</code></dt>
	<dd>full path to ssh key directory</dd>
</dl>
...
{% highlight bash %}{% raw %}
ddev auth ssh{% endraw %}{% endhighlight %}
