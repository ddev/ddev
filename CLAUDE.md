...
but `ddev-webserver`'s `start.sh` (`ddev-webserver-base-scripts/` and
`ddev-webserver-prod-scripts/`) overwrites those paths from the *project's*
`.ddev/` directory whenever it exists, wholesale, before the webserver
starts:

- `.ddev/nginx_full/` → replaces `/etc/nginx/sites-enabled`
- `.ddev/apache/` → replaces `/etc/apache2/sites-enabled`

Those project-local files are themselves generated by the `ddev` **CLI
binary**, not extracted from the container image, from a `//go:embed`d copy
under `pkg/ddevapp/webserver_config_assets/` (`nginx-site-*.conf`,
`apache-site-php.conf`, etc.), written out by `GenerateWebserverConfig()` in
`pkg/ddevapp/ddevapp.go`. They carry a `#ddev-generated` marker, so `ddev
start`/`ddev config` silently refreshes them from that embedded template —
which means the project's `.ddev/apache/apache-site.conf` (or
`nginx_full/*.conf`) always wins over whatever is baked into the image,
no matter how many times you rebuild `ddev-webserver`.

**When changing anything in a real (non-fallback) apache or nginx site
template — a `<Location>`/`SetHandler`/`Alias` block, a rewrite rule, etc. —
check whether the same content exists in `pkg/ddevapp/webserver_config_assets/`
and update it too, then rebuild the `ddev` binary (`make`), not just the
container image.** nginx mostly avoids this trap because its per-project
templates just `include /etc/nginx/common.d/*.conf;`, so fixes that live in
`common.d/*.conf` (container-image-only) reach every project without
duplication — prefer that pattern (a shared, non-duplicated include) over
editing the site templates directly when a fix can be expressed that way.
Apache has no `conf.d`-style include from its per-project templates, so
apache-level per-project fixes generally require touching both copies.

To pick up a `webserver_config_assets` change in an existing project, rebuild
the `ddev` binary and run `ddev start` (it regenerates the `#ddev-generated`
files); a project that already took ownership of the file (removed the
`#ddev-generated` line) won't be touched.
