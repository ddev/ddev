# This PowerShell script sets up a Debian WSL2 instance for use with DDEV and docker-ce installed inside WSL2.
# It requires that a Debian WSL2 distro be installed already, preferably with `wsl --install -d Debian`, but it can also be
# done manually.
...
# iex ((New-Object System.Net.WebClient).DownloadString('https://raw.githubusercontent.com/ddev/ddev/master/scripts/install_ddev_wsl2_docker_inside_debian.ps1'))
...
# Make sure default distro is a Debian release
if (-not( wsl -e grep ^NAME=.Debian //etc/os-release)) {
    throw "Your installed WSL2 distro does not seem to be Debian. You can certainly use DDEV with WSL2 in another distro, but this script is oriented to Debian."
}
...
wsl -u root bash -c "apt update && apt install -y gnupg2 apt-transport-https wget ca-certificates curl gnupg lsb-release"

wsl -u root bash -c "wget -O - https://pkg.wslutiliti.es/public.key | sudo tee -a /etc/apt/trusted.gpg.d/wslu.asc"
wsl -u root bash -c 'echo deb https://pkg.wslutiliti.es/debian $(lsb_release -cs) main | sudo tee -a /etc/apt/sources.list'

wsl -u root mkdir -p /etc/apt/keyrings
wsl -u root bash -c "rm -f /etc/apt/keyrings/docker.gpg && mkdir -p /etc/apt/keyrings && curl -fsSL https://download.docker.com/linux/debian/gpg | gpg --dearmor -o /etc/apt/keyrings/docker.gpg"
wsl -u root -e bash -c 'echo deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/debian $(lsb_release -cs) stable | tee /etc/apt/sources.list.d/docker.list > /dev/null 2>&1'
