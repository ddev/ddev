...
windows_install: $(GOTMP)/bin/windows_amd64/ddev_windows_installer.$(VERSION).exe $(GOTMP)/bin/windows_amd64/ddev_windows_user_installer.$(VERSION).exe

windows_signed: $(GOTMP)/bin/windows_amd64/ddev.exe
...
$(GOTMP)/bin/windows_amd64/ddev_windows_installer.$(VERSION).exe: windows_signed $(GOTMP)/bin/windows_amd64/ddev.exe $(GOTMP)/bin/windows_amd64/sudo.exe $(GOTMP)/bin/windows_amd64/sudo_license.txt $(GOTMP)/bin/windows_amd64/nssm.exe $(GOTMP)/bin/windows_amd64/winnfsd.exe $(GOTMP)/bin/windows_amd64/winnfsd_license.txt $(GOTMP)/bin/windows_amd64/mkcert.exe $(GOTMP)/bin/windows_amd64/mkcert_license.txt winpkg/ddev-system.nsi
	@makensis -DVERSION=$(VERSION) winpkg/ddev-system.nsi  # brew install makensis, apt-get install nsis, or install on Windows
...
$(GOTMP)/bin/windows_amd64/ddev_windows_user_installer.$(VERSION).exe: windows_signed $(GOTMP)/bin/windows_amd64/ddev.exe $(GOTMP)/bin/windows_amd64/sudo.exe $(GOTMP)/bin/windows_amd64/sudo_license.txt $(GOTMP)/bin/windows_amd64/nssm.exe $(GOTMP)/bin/windows_amd64/winnfsd.exe $(GOTMP)/bin/windows_amd64/winnfsd_license.txt $(GOTMP)/bin/windows_amd64/mkcert.exe $(GOTMP)/bin/windows_amd64/mkcert_license.txt winpkg/ddev-user.nsi
	@makensis -DVERSION=$(VERSION) winpkg/ddev-user.nsi  # brew install makensis, apt-get install nsis, or install on Windows
	@if [ -z "$(DDEV_WINDOWS_SIGNING_PASSWORD)" ] ; then echo "Skipping signing ddev_windows_user_installer, no DDEV_WINDOWS_SIGNING_PASSWORD provided"; else echo "Signing windows installer binary..."&& osslsigncode sign -pkcs12 certfiles/drud_cs.p12  -n "DDEV-Local Installer" -i https://ddev.com -in $@ -out $@ -t http://timestamp.digicert.com -pass $(DDEV_WINDOWS_SIGNING_PASSWORD); fi
	shasum -a 256 $@ >$@.sha256.txt
