...
func TestConfigValidate(t *testing.T) {
	...
	_, _, err = testcommon.GetLocalHTTPResponse("http://x.ddev.site/" + staticURI)
	assert.NoError(err)
	_, _, err = testcommon.GetLocalHTTPResponse("http://somethjingrandom.any.ddev.site/" + staticURI)
	assert.NoError(err)
