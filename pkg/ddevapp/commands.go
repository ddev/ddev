...
func PopulateGlobalCustomCommandFiles() error {
	...
	_, stderr, err := performTaskInContainer([]string{"sh", "-c", "rm -r " + path.Join(commandDirInVolume, "*") + " || true"})
	if err != nil {
		return fmt.Errorf("unable to rm %s: %v (stderr=%s)", commandDirInVolume, err, stderr)
	}
...
func performTaskInContainer(command []string) (string, string, error) {
	...
	containerName := "PopulateGlobalCustomCommandFiles" + nodeps.RandomString(12)
	uid, _, _ := util.GetContainerUIDGid()
