...
func (app *DdevApp) FixObsolete() {
	...
	if fileutil.IsDirectory(legacyCommandDir) {
		err := fileutil.PurgeDirectory(legacyCommandDir)
		if err != nil {
			util.Warning("attempted to remove files from %s but failed, you may want to remove it manually: %v", legacyCommandDir, err)
		} else {
			err = os.Remove(legacyCommandDir)
			if err != nil {
