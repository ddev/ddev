...
func (app *DdevApp) RenderComposeYAML() (string, error) {
	...
	if app.NodeJSVersion != nodeps.NodeJSDefault {
		...
		extraWebContent = extraWebContent + fmt.Sprintf("\nRUN curl -sSL --fail https://deb.nodesource.com/setup_%s.x >/tmp/setup_node.sh ||  curl -sSL --fail https://deb.nodesource.com/setup_%s.sh >/tmp/setup_node.sh", app.NodeJSVersion)
		extraWebContent = extraWebContent + "RUN bash /tmp/setup_node.sh && apt-get install nodejs && npm config set unsafe-perm true && npm install --global gulp-cli yarn"
	}
