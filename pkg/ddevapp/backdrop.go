...
func createBackdropSettingsFile(app *DdevApp) (string, error) {
	...
	if included {
		...
		if err = appendIncludeToBackdropSettingsFile(app); err != nil {
...
// getBackdropComposerCreateAllowedPaths returns fullpaths that are allowed to be present when running composer create-project
