...
//
// It also checks a directly-requested nonexistent .php file, which
// nginx-fpm and apache-fpm handle differently: nginx's `try_files $uri =404;`
// in the `\.php$` location block rejects it before it ever reaches php-fpm,
// so ddev-webserver's explanation is shown; apache's FilesMatch handler for
// *.php has no such existence check and proxies to php-fpm unconditionally,
// so php-fpm's own "No input file specified" 404 passes through unchanged
// and ddev-webserver's explanation never appears.
func TestWebserverMissingIndexExplanation(t *testing.T) {
	...
	for _, webserverType := range []string{nodeps.WebserverNginxFPM, nodeps.WebserverApacheFPM} {
		...
		require.NoError(t, err)
		if webserverType == nodeps.WebserverApacheFPM {
			require.NotContains(t, phpOut, "generated by the ddev-webserver container", "for WebserverType=%s expected php-fpm's own 404 for a nonexistent .php file, not ddev-webserver's explanation", webserverType)
			require.Empty(t, phpResp.Header.Get("X-Ddev-404-Source"), "for WebserverType=%s expected no X-Ddev-404-Source header, since php-fpm's own 404 passes through unchanged", webserverType)
		} else {
