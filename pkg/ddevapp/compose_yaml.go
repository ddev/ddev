...
func fixupComposeYaml(yamlStr string, app *DdevApp) (*composeTypes.Project, error) {
	...
	hostDockerInternalIP := dockerutil.GetHostDockerInternalIP()
	...
	for name, service := range project.Services {
		...
		if hostDockerInternalValue != "" {
			...
			if !slices.Contains(service.ExtraHosts["host.docker.internal"], hostDockerInternalValue) {
				service.ExtraHosts["host.docker.internal"] = append(service.ExtraHosts["host.docker.internal"], hostDockerInternalValue)
			}
		}
		service.Environment["HOST_DOCKER_INTERNAL_IP"] = &hostDockerInternalIP
