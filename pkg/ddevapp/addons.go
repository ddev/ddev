...
func processPHPAction(action string, installDesc InstallDesc, app *DdevApp, verbose bool) error {
	...
	// On Lima/Colima/Rancher Desktop/apple container, VirtioFS bind mounts can have a
	// brief propagation delay for newly created files. Add-on installation is infrequent
	// so a short sleep is acceptable to avoid an intermittent "file not found" race.
	if dockerutil.IsColima() || dockerutil.IsLima() || dockerutil.IsRancherDesktop() || dockerutil.IsAppleContainer() {
