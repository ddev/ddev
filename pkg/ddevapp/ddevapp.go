...
func getBackupCommand(app *DdevApp, targetFile string) string {
	...
	c := fmt.Sprintf(`%s && mariabackup --backup --stream=mbstream --user=root --password=root --socket=/var/tmp/mysql.sock 2>/tmp/snapshot_%s.log | %s > "%s"`, cleanupCmd, path.Base(targetFile), compressionCommand, targetFile)
	...
	case app.Database.Type == nodeps.MySQL:
		c = fmt.Sprintf(`%s && xtrabackup --backup --stream=xbstream --user=root --password=root --socket=/var/tmp/mysql.sock 2>/tmp/snapshot_%s.log | %s > "%s"`, cleanupCmd, path.Base(targetFile), compressionCommand, targetFile)
	case app.Database.Type == nodeps.Postgres:
		...
		if postgresDataPath != postgresDataDir {
			...
			c = fmt.Sprintf("cd %s && %s && rm -rf /var/tmp/pgbackup && pg_basebackup %s -c fast -D /var/tmp/pgbackup 2>/tmp/snapshot_%s.log && tar -I '%s' -cf %s -C /var/tmp/pgbackup/ .", postgresDataPath, cleanupCmd, walMethod, path.Base(targetFile), compressionCommand, targetFile)
		}
	}
	return c
}
