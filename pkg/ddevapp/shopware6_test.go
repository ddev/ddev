...
// for different working directory configurations
func TestShopware6SiteSettingsPaths(t *testing.T) {
	...
	// Test case 1: No working directory configured - should use AppRoot
	...
	// Test case 2: Working directory configured - should use working dir
...
func TestShopware6PostStartAction(t *testing.T) {
	...
	// Test case 1: Create .env.local in AppRoot (no working dir)
	...
	// Test case 2: Create .env.local in working directory
	t.Run("CreateInWorkingDir", func(t *testing.T) {
		// Use separate directory to avoid conflicts with previous test
		workingDirTestDir := testcommon.CreateTmpDir("CreateInWorkingDir")
		defer func() {
			_ = os.RemoveAll(workingDirTestDir)
		}()

		shopwareSubdir := filepath.Join(workingDirTestDir, "shopware")
		err := os.MkdirAll(shopwareSubdir, 0755)
		require.NoError(t, err)

		app, err := ddevapp.NewApp(workingDirTestDir, false)
		require.NoError(t, err)
		app.Name = "test-shopware6-workdir"
		app.Type = nodeps.AppTypeShopware6
		app.WorkingDir = map[string]string{
			"web": "/var/www/html/shopware",
		}
		...
		rootEnvPath := filepath.Join(workingDirTestDir, ".env.local")
...
func setShopware6SiteSettingsPaths(app *ddevapp.DdevApp) {
	// Simulate what setShopware6SiteSettingsPaths does
